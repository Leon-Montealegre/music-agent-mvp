...
export default function PromoDealPage({ params }) {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/promo-deal/files`, {
        method: 'POST',
        body: formData
      })
  ...
  const confirmDeleteFile = async () => {
    ...
    try {
      const response = await fetch(
        `${apiBase}/promo-deal/files/${encodeURIComponent(fileToDelete)}`,
        { method: 'DELETE' }
      )
  ...
  const handleDownloadFile = (filename) => {
    const url = `${apiBase}/promo-deal/files/${encodeURIComponent(filename)}`
  ...
  const handleDeleteContact = async () => {
    ...
    try {
      const response = await fetch(
        `${apiBase}/promo-deal/contacts/${contactToDelete.id}`,
        {
          method: 'DELETE'
        }
      )
