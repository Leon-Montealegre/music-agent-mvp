...
export default function CollectionPromoEntryPage({ params }) {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const [colData, promoRes, contactsData] = await Promise.all([
    ...
    <div className="min-h-screen bg-gray-900">
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="p-6">
                {(entry.contacts || []).length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <button onClick={() => setShowContactPicker(true)} className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-2 rounded-lg transition-all font-medium">
                      + Add First Contact
                    </button>
                  </div>
                ) : null}
            ...
            <div className="bg-gray-800/80 backdrop-blur-sm border border-gray-700 rounded-lg shadow-2xl">
              <div className="p-6 border-b border-gray-700">
                ...
                <p className="text-sm text-gray-400 mt-1">Use this space to track anything relevant.</p>
              </div>
              <div className="p-6 space-y-4">
                <textarea
                  ...
                  placeholder="Pricing agreed, next follow-up date, what was promised, special requirements…"
                />
        ...
        <div className="mt-8 pt-4 border-t border-gray-800 flex flex-col sm:flex-row justify-end gap-3">
          <button type="button" onClick={() => setShowEditModal(true)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-all font-medium text-sm">
            ✏️ Edit Entry
          </button>
          <button type="button" onClick={() => setShowDeleteEntryModal(true)} className="px-4 py-2 bg-red-600/20 hover:bg-red-600/30 border border-red-500/50 text-red-300 rounded-lg transition-all font-medium text-sm">
            🗑️ Delete Entry
          </button>
        </div>
      </div>
        ...
        <Modal isOpen={!!editingContact} onClose={() => setEditingContact(null)} title="Edit Contact">
          <div className="p-4 space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => setEditingContact(null)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm rounded-lg transition-colors">
                Cancel
              </button>
            </div>
          </div>
      ...
      {/* Edit Entry modal */}
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Edit Promo Entry">
        <div className="p-4 space-y-4">
          <div>
            <label className="block text-sm text-gray-300 mb-1">Promo Name <span className="text-red-400">*</span></label>
            <input type="text" value={detailsForm.promoName} onChange={e => setDetailsForm({ ...detailsForm, promoName: e.target.value })} className="w-full px-3 py-2 bg-gray-900 border border-gray-700 text-gray-100 rounded-lg focus:ring-2 focus:ring-pink-500" />
          </div>
          <div>
            <label className="block text-sm text-gray-300 mb-1">Status</label>
            <select value={detailsForm.status} onChange={e => setDetailsForm({ ...detailsForm, status: e.target.value })} className="w-full px-3 py-2 bg-gray-900 border border-gray-700 text-gray-100 rounded-lg focus:ring-2 focus:ring-pink-500">
          ...
          {detailsForm.status === 'Live' && (
            <div>
              ...
              <input type="date" value={detailsForm.liveDate} onChange={e => setDetailsForm({ ...detailsForm, liveDate: e.target.value })} className="w-full px-3 py-2 bg-gray-900 border border-gray-700 text-gray-100 rounded-lg focus:ring-2 focus:ring-pink-500" />
            </div>
          )}
          <div>
          ...
          <div>
            ...
            <textarea value={detailsForm.notes} onChange={e => setDetailsForm({ ...detailsForm, notes: e.target.value })} rows={3} className="w-full px-3 py-2 bg-gray-900 border border-gray-700 text-gray-100 rounded-lg focus:ring-2 focus:ring-pink-500" />
          </div>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={async () => { await handleSaveDetails(); setShowEditModal(false) }} disabled={savingDetails} className="flex-1 bg-pink-600 hover:bg-pink-500 text-white px-4 py-2 rounded-lg transition-all font-medium disabled:bg-gray-600 disabled:cursor-not-allowed">
              {savingDetails ? 'Saving...' : 'Save Changes'}
            </button>
            <button type="button" onClick={() => setShowEditModal(false)} className="flex-1 bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition-all font-medium">
              Cancel
            </button>
          </div>
        </div>
