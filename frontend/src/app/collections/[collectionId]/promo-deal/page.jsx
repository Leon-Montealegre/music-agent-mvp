...
export default function CollectionPromoDealPage({ params }) {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/promo-deal/files`, { method: 'POST', body: formData })
  ...
  const confirmDeleteFile = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/promo-deal/files/${encodeURIComponent(fileToDelete)}`, { method: 'DELETE' })
  ...
  const handleDownloadFile = (filename) => {
    window.open(`${apiBase}/promo-deal/files/${encodeURIComponent(filename)}`, '_blank')
  }

  const handleDeleteContact = async () => {
    if (!contactToDelete) return
    try {
      const res = await fetch(`${apiBase}/promo-deal/contacts/${contactToDelete.id}`, { method: 'DELETE' })
  ...
  const promoInfo = collection.metadata?.promoInfo || collection.promoInfo || {}
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black">
            ...
            <div className="bg-gray-800/80 backdrop-blur-sm border border-gray-700 rounded-lg shadow-2xl">
              <div className="p-6 border-b border-gray-700">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-xl font-semibold text-gray-100">Promo Contacts</h2>
                    <p className="text-sm text-gray-400 mt-1">Contacts for this promo deal</p>
                  </div>
                  <button
      ...
      >
        <LabelContactForm
          ...
          contactPath="promo-deal/contacts"
        />
      </Modal>
